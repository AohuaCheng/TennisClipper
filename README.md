...
项目支持 **两条切分路径**（输出格式相同：`work/timeline.json` → FFmpeg 导出）：

| 路径 | 触发方式 | 思路 |
|------|----------|------|
| **启发式（默认）** | `tenniscut process SESSION` | YOLO + MediaPipe 挥拍 → 击球聚类 → rally_lifecycle |
| **ML（推荐验证）** | `tenniscut process SESSION --use-ml-decoder` | YOLO → EfficientNet-B2 动作 → Set-TCN 回合概率 → 片段解码 |
...
原始视频
  ├─→ 5fps 运动扫描 (OpenCV 帧差)
  └─→ 视觉击球通道
           ├─ YOLO 球员框 + MediaPipe 挥拍峰值
           └─ 稀疏球轨迹确认（可选）
           ↓
  rally_lifecycle / ball_refine → FFmpeg 裁剪拼接
```

ML 路径详见 [`docs/ML_PIPELINE.md`](./docs/ML_PIPELINE.md)。
...
# 3. 一键处理（默认启发式；ML 路径加 --use-ml-decoder）
tenniscut process my_session

# ML 层级切分（EfficientNet-B2 + Set-TCN，见 docs/ML_PIPELINE.md）
...
| 方向 | 说明 |
|---|---|
| ML 管线优化 | Layer1 泛化、dense 采样、消融实验（见 [`docs/ML_PIPELINE.md`](./docs/ML_PIPELINE.md)） |
| 球检测微调 | YOLO ball 标注 + 微调 |
...
| Phase 3.5 | 回合 lifecycle + ball refine | ✅ 启发式 fallback |
| Phase 4 | ML 层级切分（CNN + Set-TCN） | ✅ 已接入 CLI |
| Phase 5 | 回合标签筛选 + LLM 精筛 | ⏳ 待开始 |

详细路线图见 [`docs/ROADMAP.md`](./docs/ROADMAP.md)；ML 管线见 [`docs/ML_PIPELINE.md`](./docs/ML_PIPELINE.md)。
