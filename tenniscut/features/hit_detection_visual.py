...
class PlayerTracker:
    """Simple IOU-based tracker for player bounding boxes."""
    ...
    def update(self, players: List[Dict[str, Any]], t: float) -> List[Dict[str, Any]]:
        ...
        for ti, tr in enumerate(unmatched_tracks):
            best_iou = 0.0
            best_pi = -1
        ...
        for pi, p in enumerate(players):
            if matched[pi] is None:
                ...
                matched[pi].setdefault("track_id", matched[pi]["track_id"])
                # Append historical point for this track (copy)
                track = matched[pi]
                hist = next((tr for tr in self.tracks if tr[0]["track_id"] == track["track_id"]), None)
                if hist is not None:
                    hist.append(
                        {
                            "t": t,
        ...
        # Remove stale tracks (not seen in last 2 seconds)
        self.active_tracks = [
